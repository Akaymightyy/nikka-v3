...
zokou({ nomCom: "yts", categorie: "Search", reaction: "✋" }, async (dest, zk, commandeOptions) => {
  ...
  try {
    ...
    repondre("error lors de la procédure : " + error);
  }
});
...
_*En cours de téléchargement...*_\n\n`
    ...
    */
    ...
    fileStream.on('finish', () => {
      ...
      zk.sendMessage(origineMessage, { video: { url: `./${filename}` }, caption: "Powered by *NIKKA-V3*", gifPlayback: false }, { quoted: ms });

    });

    fileStream.on('error', (error) => {
      console.error('error lors de l\'écriture du fichier vidéo :', error);
      repondre('Une error est survenue lors de l\'écriture du fichier vidéo.');
    });

  } catch (error) {
    console.error('error lors de la recherche ou du téléchargement de la vidéo :', error);
    repondre('Une error est survenue lors de la recherche ou du téléchargement de la vidéo.' + error);
  }
});
...
zokou({
  ...
  try {
    ...
    fileStream.on('error', (error) => {
      console.error('error lors de l\'écriture du fichier audio :', error);
      repondre('Une error est survenue lors de l\'écriture du fichier audio.');
    });

  } catch (error) {
    console.error('error lors de la recherche ou du téléchargement de la vidéo :', error);
    repondre('Une error est survenue lors de la recherche ou du téléchargement de la vidéo.');
  }
});
