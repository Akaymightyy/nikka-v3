...
const creerTableHentai = async () => {
  try {
    ...
    console.error("Une error est survenue lors de la création de la table 'hentai':", e);
  }
};
...
async function addToHentaiList(groupeJid) {
  ...
  try {
    ...
    console.error("error lors de l'ajout du groupe à la liste de hentai :", error);
  } finally {
    client.release();
  }
}
...
async function checkFromHentaiList(groupeJid) {
  ...
  try {
    ...
    console.error("error lors de la vérification de la présence du groupe dans la liste de hentai :", error);
    return false;
  } finally {
    client.release();
  }
...
async function removeFromHentaiList(groupeJid) {
  ...
  try {
    ...
    console.error("error lors de la suppression du groupe de la liste de hentai :", error);
  } finally {
    client.release();
  }
}
