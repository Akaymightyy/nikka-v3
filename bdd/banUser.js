...
const creerTableBanUser = async () => {
  try {
    ...
    console.error("Une error est survenue lors de la création de la table 'banUser':", e);
  }
};
...
async function addUserToBanList(jid) {
  ...
  try {
    ...
    console.error("error lors de l'ajout de l'utilisateur banni :", error);
  } finally {
    client.release();
  }
}
...
async function isUserBanned(jid) {
  ...
  try {
    ...
    console.error("error lors de la vérification de l'utilisateur banni :", error);
    return false;
  } finally {
    client.release();
  }
...
async function removeUserFromBanList(jid) {
  ...
  try {
    ...
    console.error("error lors de la suppression de l'utilisateur banni :", error);
  } finally {
    client.release();
  }
}
