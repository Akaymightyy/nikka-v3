...
async function creerTableWarnUsers() {
    ...
    try {
      ...
      console.error("error lors de la création de la table 'warn_users':", error);
    } finally {
      client.release();
    }
  };
   ...
   async function ajouterUtilisateurAvecWarnCount(jid) {
    ...
    try {
      ...
      console.error("error lors de l'ajout ou de la mise à jour de l'utilisateur :", error);
    } finally {
      client.release();
    }
  } ;
  ...
  async function getWarnCountByJID(jid) {
    ...
    try {
      ...
      console.error("error lors de la récupération du warn_count :", error);
      return -1; // Retournez une valeur d'error ou une autre valeur par défaut en cas d'error
    } finally {
      client.release();
    }
  } ;
  ...
  async function resetWarnCountByJID(jid) {
    ...
    try {
      ...
      console.error("error lors de la réinitialisation du warn_count :", error);
    } finally {
      client.release();
    }
  }
