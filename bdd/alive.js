...
const creerTableAlive = async () => {
    try {
      ...
      console.error("Une error est survenue lors de la création de la table 'alive':", e);
    }
  };
...
async function addOrUpdateDataInAlive(message, lien) {
    ...
    try {
      ...
      console.error("error lors de l'ajout ou de la mise à jour des données dans la table 'alive':", error);
    } finally {
      client.release();
    }
  };
  ...
  async function getDataFromAlive() {
    ...
    try {
      ...
      console.error("error lors de la récupération des données depuis la table 'alive':", error);
      return null;
    } finally {
      client.release();
    }
