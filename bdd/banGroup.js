...
const creerTableBanGroup = async () => {
  try {
    ...
    console.error("Une error est survenue lors de la création de la table 'banGroup':", e);
  }
};
...
async function addGroupToBanList(groupeJid) {
  ...
  try {
    ...
    console.error("error lors de l'ajout du groupe banni :", error);
  } finally {
    client.release();
  }
}
...
async function removeGroupFromBanList(groupeJid) {
  ...
  try {
    ...
    console.error("error lors de la suppression du groupe banni :", error);
  } finally {
    client.release();
  }
}
