...
async function createSudoTable() {
  ...
  try {
    ...
    console.error("Une error est survenue lors de la création de la table 'sudo':", error);
  } finally {
    client.release();
  }
}
...
async function issudo(jid) {
    ...
    try {
      ...
      console.error("error lors de la vérification du groupe banni :", error);
      return false;
    } finally {
      client.release();
    }
  ...
  async function removeSudoNumber(jid) {
    ...
    try {
      ...
      console.error("error lors de la suppression du numéro de téléphone autorisé :", error);
    } finally {
      client.release();
    }
  }
  ...
  async function addSudoNumber(jid) {
    ...
    try {
      ...
      console.error("error lors de l'ajout du numéro de téléphone autorisé :", error);
    } finally {
      client.release();
    }
  }
  ...
  async function getAllSudoNumbers() {
    ...
    try {
      ...
      console.error("error lors de la récupération des numéros de téléphone autorisés :", error);
      return [];
    } finally {
      client.release();
    }
    ...
    try {
      ...
      console.error('error lors de la vérification de la table "sudo" :', error);
      return false; // En cas d'error, considérez la table comme vide
    } finally {
      client.release();
    }
  };
