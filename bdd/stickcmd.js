...
async function creerTableStickcmd() {
  try {
    ...
    console.error("Une error est survenue lors de la création de la table 'stickcmd':", e);
  }
}
...
async function addstickcmd(cmd, id) {
  ...
  try {
    ...
    console.log('error lors de l\'ajout du stickcmd', error);
  } finally {
    if (client) {
      client.release();
    }
...
async function deleteCmd(cmd) {
  ...
  try {
    ...
    console.error("error lors de la suppression du stickcmd :", error);
  } finally {
    client.release();
  }
} ;
...
async function getCmdById(id) {
    ...
    try {
      ...
      console.error("error lors de la récupération du stickcmd par id :", error);
      return null; // Gérer l'error et ajuster la valeur de retour si nécessaire.
    } finally {
      if (client) {
        client.release();
      }
  ...
  async function getAllStickCmds() {
    ...
    try {
        ...
        console.error("error lors de la récupération de toutes les commandes stickcmd :", error);
        return [];
    } finally {
        client.release();
    }
