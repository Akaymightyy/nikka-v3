...
async function creerTableMention() {
  ...
  try {
      ...
      console.error("Une error est survenue lors de la création de la table 'mention':", e);
    } finally {
        client.release();
      }
  };
...
creerTableMention();
  ...
  async function addOrUpdateDataInMention(url, type,message) {
      ...
      try {
          ...
          console.error("error lors de l'ajout ou de la mise à jour des données dans la table 'mention':", error);
      } finally {
          client.release();
      }
  };
...
async function modifierStatusId1(nouveauStatus) {
    ...
    try {
        ...
        console.error("error lors de la modification du status pour l'ID 1 dans la table 'mention':", error);
    } finally {
        client.release();
    }
};
...
async function recupererToutesLesValeurs() {
    ...
    try {
        ...
        console.error("error lors de la récupération des valeurs de la table 'mention':", error);
    } finally {
        client.release();
    }
};
