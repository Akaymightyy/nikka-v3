...
async function createTablecron() {
    ...
    try {
      ...
      console.error("Une error est survenue lors de la création de la table 'cron':", error);
    } finally {
      client.release();
    }
} ;
...
async function getCron() {
  ...
  try {
    ...
    console.error('error lors de la récupération des données de la table "cron":', error);
  } finally {
    client.release();
  }
 }  ;
...
if (exist) {
    ...
    console.error('error lors de l\'ajout de la donnée dans la table "cron":', error);
  } finally {
    client.release();
  }
}
...
async function delCron(group_id) {
  ...
  try {
    ...
    console.error('error lors de la suppression de la donnée dans la table "cron":', error);
  } finally {
    client.release();
  }
}
