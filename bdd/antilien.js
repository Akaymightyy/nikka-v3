...
async function createAntilienTable() {
  ...
  try {
    ...
    console.error("Une error est survenue lors de la création de la table 'antilien':", error);
  } finally {
    client.release();
  }
}
...
async function ajouterOuMettreAJourJid(jid, etat) {
  ...
  try {
    ...
    console.error('error lors de l\'ajout ou de la mise à jour du JID dans la table ,', error);
  } finally {
    client.release();
  }
};
...
async function mettreAJourAction(jid, action) {
  ...
  try {
    ...
    console.error('error lors de la mise à jour de l\'action pour le JID dans la table  :', error);
  } finally {
    client.release();
  }
};
...
async function verifierEtatJid(jid) {
  ...
  try {
    ...
    console.error('error lors de la vérification de l\'état du JID dans la table ', error);
    return false;
  } finally {
    client.release();
  }
...
async function recupererActionJid(jid) {
  ...
  try {
    ...
    console.error('error lors de la récupération de l\'action du JID dans la table :', error);
    return 'supp'; // Gestion de l'error en retournant une valeur par défaut
  } finally {
    client.release();
  }
};
