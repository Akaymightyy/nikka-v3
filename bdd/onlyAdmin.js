...
const creerTableOnlyAdmin = async () => {
  try {
    ...
    console.error("Une error est survenue lors de la création de la table 'onlyAdmin':", e);
  }
};
...
async function addGroupToOnlyAdminList(groupeJid) {
  ...
  try {
    ...
    console.error("error lors de l'ajout du groupe onlyAdmin :", error);
  } finally {
    client.release();
  }
}
...
async function isGroupOnlyAdmin(groupeJid) {
  ...
  try {
    ...
    console.error("error lors de la vérification du groupe onlyAdmin :", error);
    return false;
  } finally {
    client.release();
  }
...
async function removeGroupFromOnlyAdminList(groupeJid) {
  ...
  try {
    ...
    console.error("error lors de la suppression du groupe onlyAdmin :", error);
  } finally {
    client.release();
  }
}
