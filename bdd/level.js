...
async function createUsersRankTable() {
  ...
  try {
    ...
    console.error('error lors de la création de la table users_rank:', error);
  } finally {
    client.release();
  }
}
...
async function ajouterOuMettreAJourUserData(jid) {
  ...
  try {
    ...
    console.error('error lors de la mise à jour des données de l\'utilisateur:', error);
  } finally {
    client.release();
  }
};
...
async function getMessagesAndXPByJID(jid) {
  ...
  try {
    ...
    console.error('error lors de la récupération des données de l\'utilisateur:', error);
    return { messages: 0, xp: 0 }; // En cas d'error, renvoyez des valeurs par défaut
  } finally {
    client.release();
  }
}
...
async function getBottom10Users() {
  ...
  try {
    ...
    console.error('error lors de la récupération du bottom 10 des utilisateurs:', error);
    return []; // En cas d'error, renvoyez un tableau vide
  } finally {
    client.release();
  }
}
